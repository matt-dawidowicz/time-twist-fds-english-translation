...
Use `python -m black src tools tests` or
`python -m ruff check --fix src tools tests` only for mechanical corrections,
then review the diff.

## Public tests versus private maintainer tests

`python -m tools.maintenance.run_tests unit` is fixture-free and runs without
game data. `python -m tools.maintenance.run_tests integration` checks exact
ROM-derived transforms and requires a separately maintained private fixture
overlay. Missing fixtures are an expected setup limitation, not a reason to add
skips or weaken checks.
...
| Area | Purpose |
| --- | --- |
| `src/time_twist/` | Installable, release-critical runtime and binary transformation code |
| `tools/` | Developer analysis, audit, generation, maintenance, and preview tooling |
| `tests/unit/` | Fixture-free public regression tests |
| `tests/integration/` | Exact ROM-derived maintainer tests requiring the private overlay |
| `tests/fixtures/` | Public metadata describing the private fixture contract |
| `work/` | Project data, translation sources, generated review banks, manifests, and evidence |
| `outputs/` | Committed human-review artifacts regenerated by CI |

`work/` is deliberately not a Python package. New executable code belongs in
`src/time_twist/` when it is release-critical, or under the appropriate
`tools/` category when it is developer-facing.

## Module map

| Area | Main module |
| --- | --- |
| Command-line entry point | `src/time_twist/cli.py` (`cli_commands.py`, `cli_parser.py`) |
| FDS parsing and rebuilding | `src/time_twist/fds.py` |
| Packed symbols and native codec | `src/time_twist/textcodec.py` |
| English encoding and validation | `src/time_twist/english.py`, `scenario_validation.py` |
| Scenario parsing/recompression | `src/time_twist/scenario.py`, `compression.py` |
| Fixed UI patches and data | `src/time_twist/ui.py`, `ui_fixed_tables.py` |
| Font and title patches | `src/time_twist/font.py`, `title.py`, `title_layout.py`, `title_assets.py`, `title_patch.py` |
| Candidate/release assembly | `src/time_twist/release.py`, `release_metadata.py` |
...
Every maintained Python module, class, function, and method must have a purpose
docstring. Pydocstyle is the single authoritative documentation-style gate; do
not duplicate its policy in ad hoc source-scanning tests. Explain non-obvious
lines with comments that answer **why** a recovered address, byte sequence,
capacity limit, or fail-closed condition exists. Do not pad routine syntax with
line-by-line narration; it makes the binary constraints harder to find.
