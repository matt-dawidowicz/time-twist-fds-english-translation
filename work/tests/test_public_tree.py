...
class PublicTreePolicyTests(unittest.TestCase):
    ...
    def test_private_artifacts_and_personal_paths_are_rejected(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            personal_path = "C:" + "\\Users\\" + "developer\\Downloads\\review.html"
            (root / "notes.txt").write_text(personal_path, encoding="utf-8")
            problems = check_public_tree(root)
            self.assertTrue(any("private.fds" in problem for problem in problems))
            self.assertTrue(any("machine-local absolute path" in problem for problem in problems))
