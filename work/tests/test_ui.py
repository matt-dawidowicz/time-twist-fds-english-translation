...
class StaticUiTests(unittest.TestCase):
    def test_kouhen_direct_boot_guard_is_horizontal_english(self) -> None:
        path = Path("work/extracted_kouhen/side0_01_SON-KOUH_DD1D.bin")
        if not path.exists():
            self.skipTest("Kouhen direct-boot fixture is not available")
    ...
    def test_zenpen_nov2_start_prompt_patch(self) -> None:
        path = Path("work/extracted_zenpen/side0_06_NOV2_6000.bin")
        if not path.exists():
    ...
    def test_translated_tt1a_contains_english_blood_type_choices(self) -> None:
        path = Path("work/translated_banks/TT1A_fixed_footprint.bin")
        if not path.exists():
            self.skipTest("translated TT1A fixture is not available")
    ...
    def test_tt1b_fixed_menu_and_object_table_is_size_neutral(self) -> None:
        path = Path("work/build/TT1B_english_scenario.bin")
        if not path.exists():
            self.skipTest("translated TT1B scenario fixture is not available")
        original = path.read_bytes()
    ...
    def test_tt2_fixed_menu_and_quiz_table_is_size_neutral(self) -> None:
        path = Path("work/build/TT2_english_scenario.bin")
        if not path.exists():
            self.skipTest("translated TT2 scenario fixture is not available")
        original = path.read_bytes()
    ...
    def test_t22_fixed_menu_and_object_table_is_size_neutral(self) -> None:
        path = Path("work/build/T22_english_scenario.bin")
        if not path.exists():
            self.skipTest("translated T22 scenario fixture is not available")
        original = path.read_bytes()
    ...
    def test_tt3a_fixed_menu_and_quiz_table_is_size_neutral(self) -> None:
        path = Path("work/build/TT3A_english_scenario.bin")
        if not path.exists():
            self.skipTest("translated TT3A scenario fixture is not available")
        original = path.read_bytes()
    ...
    def test_tt3b_fixed_menu_and_battle_table_is_size_neutral(self) -> None:
        path = Path("work/build/TT3B_english_scenario.bin")
        if not path.exists():
            self.skipTest("translated TT3B scenario fixture is not available")
        original = path.read_bytes()
    ...
    def test_tt3_fixed_tables_reject_unknown_sources(self) -> None:
        fixtures = (
            (
                Path("work/build/TT3A_english_scenario.bin"),
                TT3A_FIXED_TEXT_START_OFFSET,
                patched_tt3a_ui,
            ),
            (
                Path("work/build/TT3B_english_scenario.bin"),
                TT3B_FIXED_TEXT_START_OFFSET,
                patched_tt3b_ui,
            ),
        )
        if not all(path.exists() for path, _, _ in fixtures):
            self.skipTest("translated TT3 fixtures are not available")
    ...
    def test_tt4_fixed_table_preserves_every_record_address(self) -> None:
        path = Path("work/build/TT4_english_scenario.bin")
        if not path.exists():
            self.skipTest("translated TT4 scenario fixture is not available")
        original = path.read_bytes()
    ...
    def test_tt5_fixed_table_preserves_every_record_address(self) -> None:
        path = Path("work/build/TT5_english_scenario.bin")
        if not path.exists():
            self.skipTest("translated TT5 scenario fixture is not available")
        original = path.read_bytes()
    ...
    def test_t25_fixed_table_preserves_every_record_address(self) -> None:
        path = Path("work/build/T25_english_scenario.bin")
        if not path.exists():
            self.skipTest("translated T25 scenario fixture is not available")
        original = path.read_bytes()
    ...
    def test_tt6_fixed_tables_preserve_every_record_address(self) -> None:
        ...
        for bank_name, start, end_offset, labels, patcher in fixtures:
            path = Path(f"work/build/{bank_name}_english_scenario.bin")
            if not path.exists():
                self.skipTest(f"translated {bank_name} fixture is not available")
