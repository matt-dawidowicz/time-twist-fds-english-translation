...
def decode_symbol(reader: BitReader) -> Symbol:
    ...
    if first == 0 or second == 0:
        return Symbol("common", (first << 5) | (second << 4) | reader.read_bits(4))
...
def decode_fixed_records(data: bytes, offset: int, count: int) -> tuple[list[list[Symbol]], int]:
...
def decode_external_bank(data: bytes, group_counts: Iterable[int]) -> tuple[list[list[list[Symbol]]], list[list[Symbol]]]:
    ...
    max_dictionary_index = max(
        (symbol.value for group in groups for record in group for symbol in record if symbol.kind == "dictionary"),
        default=0,
    )
    ...
    dictionary, _ = decode_fixed_records(data, dictionary_offset, max_dictionary_index)
    nested_max = max(
        (symbol.value for record in dictionary for symbol in record if symbol.kind == "dictionary"),
        default=0,
    )
...
def render_record(record: list[Symbol], dictionary: list[list[Symbol]], *, stack: tuple[int, ...] = ()) -> str:
    rendered: list[str] = []
    for symbol in record:
        if symbol.kind == "common":
            ...
            rendered.append(render_record(dictionary[index], dictionary, stack=(*stack, index)))
        else:
            raise ValueError(f"unexpected symbol kind in record: {symbol.kind}")
    return "".join(rendered)


def patch_spans(payload: bytes, *, max_gap: int = PATCH_GAP) -> list[tuple[int, int]]:
    """Infer patch write spans from a sparse absolute-offset payload.

    The observed patch writer coalesces non-zero changed bytes when no more than
    five bytes lie between them.  Returning full spans preserves intentional
    zero writes inside each hunk.
    """
...
def overlay_sparse_payload(base: bytes, payload: bytes, *, max_gap: int = PATCH_GAP) -> bytes:
    ...
    for side_index in range(len(raw) // SIDE_SIZE):
        ...
        for _ in range(side[57]):
            ...
            if len(header) != 16 or header[0] != 3:
                raise ValueError(f"invalid file header on side {side_index} at {position}")
            ...
            if position >= len(side) or side[position] != 4:
                raise ValueError(f"missing data block on side {side_index} at {position}")
