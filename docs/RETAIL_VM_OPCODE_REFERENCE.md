...
- `B7 configure_hotspot_boundaries` writes actor/hotspot count `$07AC`, lower boundary selector `$07AA`, and upper selector `$07AB=operand+1`. The scene hotspot sentinels are now directionally resolved: `$FD` is the left boundary (blocks outward Left, permits inward Right) and `$FE` is the right boundary (blocks outward Right, permits inward Left).
